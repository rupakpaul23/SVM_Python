#Technological Error demonstration in Channel Encoding Block
import pandas as pd
import numpy as np
...
for _ in range(num_simulations):
    ...
    # Function to flip  nth bits of a binary string, #878 for 30% and 1463 for 50% anomalies
    data2.enc_data2.iloc[:1463] = data2.enc_data2.iloc[:1463].str.slice(stop=0) + (1 - data2.enc_data2.iloc[:1463].str.get(0).astype(int)).astype(str) + data2.enc_data2.iloc[:1463].str.slice(start=1)
    #data2.enc_data2.iloc[:1463] = data2.enc_data2.iloc[:1463].str.slice(stop=7) + (1 - data2.enc_data2.iloc[:1463].str.get(7).astype(int)).astype(str) + data2.enc_data2.iloc[:1463].str.slice(start=8)
    #data2.enc_data2.iloc[:1463] = data2.enc_data2.iloc[:1463].str.slice(stop=13) + (1 - data2.enc_data2.iloc[:1463].str.get(13).astype(int)).astype(str) + data2.enc_data2.iloc[:1463].str.slice(start=14)
...
print(f"The linear_accuracy average of the {num_simulations} simulations is: {average1}")
print(f"The rbf_accuracy average of the {num_simulations} simulations is: {average2}")
print(f"The linear_accuracy_test_std_scaled average of the {num_simulations} simulations is: {average3}")
